...
class GraphicSkin:Skin{
    ...
    override func draw(){
        ...
        if(hasStateChanged || hasSizeChanged || hasStyleChanged){
            ...
            for (var depth : Int = 0; depth < depthCount; depth++) {
                if(hasSizeChanged){Utils.size(decoratables[depth] as! ISizeable, CGSize(width!/*+ padding.left + padding.right*/,height!/*+ padding.top + padding.bottom*/))}//do sizing of the sizable here
