...
class TreeModifier {
    ...
    static func remove(_ tree: inout Tree,_ idx3d:[Int]){
        ...
        if let parent:Tree = tree[parentTreeIdx3d], let last:Int = idx3d.last,  parent.children[safe:last] != nil {//use <- has here
            Swift.print("parent.children.count: \(tree[parentTreeIdx3d]?.count) last: \(idx3d.last)")
