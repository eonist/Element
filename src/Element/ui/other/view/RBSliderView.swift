...
class RBSliderView:SliderView, IRBSlidable/*:SliderView,*/ {
    ...
    override func scrollWheel(with event: NSEvent) {
        Swift.print("RBSliderView.scrollWheel() event.deltaY: \(event.deltaY)")
        (self as IRBScrollable).scroll(event)//forward the event to the scrollExtension
        if(event.phase == NSEventPhase.changed){(self as IRBSlidable).setProgress(mover!.result)}/*direct manipulation*/
