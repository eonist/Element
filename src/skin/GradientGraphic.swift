...
class GradientGraphic:PositionalDecorator/*<--recently changed from GraphicDecoratable, and then changed to SizeableDec..was positionalDec etc, then changed back to positional*/ {//TODO: probably should extend SizeableDecorator, so that we can resize the entire Decorator structure
    /**
     *
     */
    ...
    override func applyLineStyle() {
        ...
        if(getGraphic().lineStyle!.dynamicType is GradientLineStyle.Type){//<--the dynamicType may not be needed
            ...
            //TODO: the above isn't totally correct, use the outlinestroke method and then get the boundingbox from that, think different caps etc
